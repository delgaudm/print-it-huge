...
export default function App() {
  ...
  const calculatePagesFromWall = (wallWidth: number, wallHeight: number) => {
    ...
    return {
      pagesWide: clampPageCount(calculatedPagesWide),
      pagesHigh: clampPageCount(calculatedPagesHigh)
    };
  };
  ...
  const getLayoutMath = () => {
    ...
    return { pWidth, pHeight, gridW: pagesWide, gridH: pagesHigh, totalWidthMm, totalHeightMm, dotSize, cols, rows, offsetX: 0, offsetY: 0, contentPages, colsPerPage, rowsPerPage };
  };
  ...
  useEffect(() => {
    ...
    if (style === 'cmyk' && cmykData) {
      ...
      drawImageFitted(ctx, image, canvas.width, canvas.height, preserveAspectRatio ? 'contain' : 'cover');
  ...
  const previewWallDimensions = calculateWallFromPages(pagesWide, pagesHigh);
  const previewUnitLabel = wallUnit === 'imperial' ? 'ft' : 'mm';
  const previewWidthLabel = wallUnit === 'imperial'
    ? previewWallDimensions.width.toFixed(1)
    : previewWallDimensions.width.toFixed(0);
  const previewHeightLabel = wallUnit === 'imperial'
    ? previewWallDimensions.height.toFixed(1)
    : previewWallDimensions.height.toFixed(0);
              ...
              <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full border-2 border-[#1a1a1a] p-1.5 text-xs font-medium outline-none transition-all focus:shadow-[1.5px_1.5px_0px_0px_rgba(26,26,26,1)] ${darkMode ? 'bg-[#1a1a1a] text-[#fff]' : 'bg-white text-[#1a1a1a]'}`}
                        value={pagesWide}
                        onChange={(e) => setPagesWide(clampPageCount(parseInt(e.target.value) || 1))}
                      />
                    </div>
                    <div>
                      <label className={`block text-xs font-bold mb-0.5 uppercase tracking-wide ${darkMode ? 'text-[#999]' : 'text-[#1a1a1a]/70'}`}>Pages High</label>
                      <input
                        aria-label="Pages High"
                        type="number"
                        min="1"
                        max={MAX_PAGES_PER_SIDE}
                        className={`w-full border-2 border-[#1a1a1a] p-1.5 text-xs font-medium outline-none transition-all focus:shadow-[1.5px_1.5px_0px_0px_rgba(26,26,26,1)] ${darkMode ? 'bg-[#1a1a1a] text-[#fff]' : 'bg-white text-[#1a1a1a]'}`}
                        value={pagesHigh}
                        onChange={(e) => setPagesHigh(clampPageCount(parseInt(e.target.value) || 1))}
                      />
                    </div>
                  </div>
                )}
                      ...
                      <>
                        ...
                        {isTooMany && (
                          <p className="text-[10px] font-bold mb-1 text-[#e63946]">
                            ⚠️ Posters are limited to {MAX_PAGES_PER_SIDE} × {MAX_PAGES_PER_SIDE} pages
                          </p>
                        )}
                ...
                <div className={`p-1.5 border-2 border-[#1a1a1a] ${darkMode ? 'bg-[#1a1a1a]/50' : 'bg-[#f5f0e6]/50'}`}>
                  ...
                  <select
                    ...
                    className={`w-full border-2 border-[#1a1a1a] p-1.5 text-xs font-medium outline-none ${darkMode ? 'bg-[#1a1a1a] text-white' : 'bg-white text-[#1a1a1a]'}`}
                  ...
                  <p className={`mt-1 text-[10px] ${darkMode ? 'text-[#777]' : 'text-[#1a1a1a]/55'}`}>
                    {preserveAspectRatio
            ...
            <div className={`${darkMode ? 'bg-[#1a1a1a]' : 'bg-white'} border-3 border-[#1a1a1a] shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] p-2 h-full flex flex-col relative`}>
              ...
              >
                {!image ? (
                  ...
                  <div
                    className="relative pt-10 pr-16 transition-transform duration-75 ease-out"
                    style={{
                      transform: `scale(${zoom}) translate(${pan.x / zoom}px, ${pan.y / zoom}px)`,
                      cursor: zoom > 1 ? (isPanning ? 'grabbing' : 'grab') : 'default'
                    }}
                  >
                    <div className="absolute left-0 right-16 top-1 h-8 flex items-center pointer-events-none" aria-hidden="true">
                      <div className={`absolute left-0 right-0 top-1/2 border-t-2 ${darkMode ? 'border-[#faf8f3]' : 'border-[#1a1a1a]'}`} />
                      <div className={`absolute left-0 top-1 bottom-1 border-l-2 ${darkMode ? 'border-[#faf8f3]' : 'border-[#1a1a1a]'}`} />
                      <div className={`absolute right-0 top-1 bottom-1 border-r-2 ${darkMode ? 'border-[#faf8f3]' : 'border-[#1a1a1a]'}`} />
                      <span className={`relative z-10 mx-auto px-2 text-sm font-bold font-mono ${darkMode ? 'bg-[#0f0f0f] text-[#faf8f3]' : 'bg-[#f5f0e6] text-[#1a1a1a]'}`}>
                        {previewWidthLabel} {previewUnitLabel}
                      </span>
                    </div>

                    <div className="absolute right-1 top-10 bottom-0 w-12 flex items-center justify-center pointer-events-none" aria-hidden="true">
                      <div className={`absolute top-0 bottom-0 left-1/2 border-l-2 ${darkMode ? 'border-[#faf8f3]' : 'border-[#1a1a1a]'}`} />
                      <div className={`absolute left-2 right-2 top-0 border-t-2 ${darkMode ? 'border-[#faf8f3]' : 'border-[#1a1a1a]'}`} />
                      <div className={`absolute left-2 right-2 bottom-0 border-b-2 ${darkMode ? 'border-[#faf8f3]' : 'border-[#1a1a1a]'}`} />
                      <span className={`relative z-10 px-1 py-2 text-sm font-bold font-mono whitespace-nowrap ${darkMode ? 'bg-[#0f0f0f] text-[#faf8f3]' : 'bg-[#f5f0e6] text-[#1a1a1a]'}`}>
                        {previewHeightLabel} {previewUnitLabel}
                      </span>
                    </div>

                    <div className="shadow-lg border-4 border-[#1a1a1a] bg-white">
                      <canvas
                        ref={canvasRef}
                        className="block"
                      />
